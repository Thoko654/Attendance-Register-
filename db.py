...
def norm_barcode(b) -> str:
    """Normalize barcode by stripping spaces and leading zeros."""
    if b is None:
        return ""
    s = str(b).strip()
...
def get_learners_df(db_path: Path) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        # Ensure consistent barcode formatting
        df["Barcode"] = df["Barcode"].astype(str).apply(norm_barcode)
    return df
...
def delete_learner_by_barcode(db_path: Path, barcode: str) -> int:
    ...
    # (Recommended) also delete their data
    cur.execute("DELETE FROM attendance WHERE Barcode = ?", (nb,))
    cur.execute("DELETE FROM inout_log WHERE barcode = ?", (nb,))
...
def add_class_date(db_path: Path, date_label: str):
    # No-op for SQLite (dates appear once scans are recorded)
    return
...
def get_wide_sheet(db_path: Path) -> pd.DataFrame:
    ...
    att["Barcode"] = att["Barcode"].astype(str).apply(norm_barcode)
...
def append_inout_log(db_path: Path, ts_iso: str, date_str: str, time_str: str,
    ...
    # ✅ FIX: 7 placeholders for 7 columns
    cur.execute("""
...
def get_currently_in(db_path: Path, date_str: str) -> pd.DataFrame:
    ...
    latest = df.sort_values("ts_iso").groupby("barcode", as_index=False).tail(1)
    latest_in = latest[latest["action"] == "IN"].copy()

    return latest_in[["barcode", "name", "surname"]].rename(
...
def seed_learners_from_csv_if_empty(db_path: Path, csv_path: str):
    ...
    csv_df = csv_df[["Name", "Surname", "Barcode", "Grade", "Area", "Date Of Birth"]].copy()
    csv_df["Barcode"] = csv_df["Barcode"].astype(str).apply(norm_barcode)
    csv_df = csv_df[csv_df["Barcode"].astype(str).str.strip() != ""].reset_index(drop=True)
