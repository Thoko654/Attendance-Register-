# app.py — Streamlit Attendance (beautiful edition)
...
def mark_present(barcode: str, csv_path: Path) -> tuple[bool, str]:
    ...
    if not matches:
        return False, ("Barcode not found in sheet. Add this code to the 'Barcode' column for the correct learner (Manage tab).")
    ...
    for i in matches:
        ...
        if str(df.at[i, today_col]).strip() == "1":
            msgs.append(f"ℹ️ {who} already present for {today_col}.")
        else:
            df.at[i, today_col] = "1"
            updated_any = True
...
with tabs[0]:
    ...
    with c2:
        st.caption("Scanners should send Enter/Return after each scan (most do by default).")
    ...
    if not csv_path.exists():
        ...
        if not date_cols:
            ...
            if not metrics.empty:
                ...
                # Render progress bars in a grid
                ...
                # Quick summaries (top 10 consistency)
